...
public class TypeDecoderAdapt {
    ...
    static Type instantiateArrayType(TypeReference ref, Object value)
        ...
        TypeReference subTypeReference = null;
        for (Object o : values) {
    ...
    private static <T extends Type> T decodeArrayElements(
        ...
        try {
            ...
            if (Array.class.isAssignableFrom(cls)) {
                ...
                int currOffset = offset+MAX_BYTE_LENGTH_FOR_HEX_STRING*length;
                int lastLength = decodeUintAsInt(input,offset);
                for(int i = 0; i<length;i++) {
                    T value = decode(input, currOffset, cls);
                    elements.add(value);
                    int currLength = decodeUintAsInt(input, offset + MAX_BYTE_LENGTH_FOR_HEX_STRING * (i+1));
                    currOffset+=(currLength-lastLength)*2;
